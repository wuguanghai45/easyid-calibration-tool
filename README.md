...
产线 Web 服务启动后会持续监听串口输出，监听过程不会向扫描器发送开灯或扫码指令。操作员按下扫描器实体按钮后，扫描器输出车架二维码，页面随即自动触发 IMV 标定与飞书同步，无需先点击按钮。原有手动流程保持不变：点击 **开始标定** 后发送 `\\x16T\\r` 开扫码、等待一行结果，再发送 `\\x16U\\r` 关闭。扫码结果会覆盖输入框已有内容；取消勾选“使用扫描器获取车架号并自动触发标定”后，可改用手动输入。
...
| API | 说明 |
|-----|------|
| `GET /api/vin/status` | 查询串口监听连接状态和最新扫码序号 |
| `POST /api/vin/scan` | 默认执行原有主动扫码；`passive=true` 时仅监听实体按钮输出，`after` 指定扫码序号游标 |
