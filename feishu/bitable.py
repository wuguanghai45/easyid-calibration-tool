"""Bitable API: search records by S/N and update cameraOffsetTheta."""
...
def search_record_by_sn(
    ...
    """Find exactly one record by S/N*; return record_id."""
    path = f"/bitable/v1/apps/{app_token}/tables/{table_id}/records/search"
    body: dict[str, Any] = {
        "automatic_fields": False,
        "field_names": [SN_FIELD, MODEL_FIELD, THETA_FIELD],
        "filter": {
            "conjunction": "and",
            "conditions": [
                {
    ...
    if not items:
        raise FeishuApiError.business(
            "search",
            f"未找到 S/N={sn!r} 的记录（total={total}）。"
            "请检查序列号、高级权限及字段名是否与表头完全一致。",
        )
    if len(items) > 1:
        raise FeishuApiError.business(
            "search",
            f"匹配到 {len(items)} 条 S/N={sn!r} 的记录，拒绝更新以避免误写。",
        )
...
def update_camera_offset_theta(
    ...
    for value in candidates:
        try:
            return _put_theta(client, path, value)
    ...
    if last_error is not None:
        raise last_error
    raise FeishuApiError.business("update", "无法写入 cameraOffsetTheta(°)")
