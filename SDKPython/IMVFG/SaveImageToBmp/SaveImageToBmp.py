...
def saveImageToBmp(cam, frame):
    ...
    if ((frame.frameInfo.pixelFormat != IMV_FG_EPixelType.IMV_FG_PIXEL_TYPE_Mono8)
        ...
        stPixelConvertParam.eBayerDemosaic = IMV_FG_EBayerDemosaic.IMV_FG_DEMOSAIC_EDGE_SENSING
...
def mallocConvertBuffer(cam):
    ...
    if (IMV_FG_EPixelType.IMV_FG_PIXEL_TYPE_Mono8 == pixelFormatVal) | (IMV_FG_EPixelType.IMV_FG_PIXEL_TYPE_BGR8 == pixelFormatVal):
        """ // mono8和BGR8裸数据不需要转码 """
        return IMV_FG_OK
    nRet = cam.IMV_FG_GetIntFeatureValue("Width", widthVal)
    if IMV_FG_OK != nRet:
        print("Get Width feature value failed! ErrorCode[%d]" % nRet)
        return nRet
    nRet = cam.IMV_FG_GetIntFeatureValue("Height", heightVal)
    if IMV_FG_OK != nRet:
...
if __name__ == "__main__":
    ...
    # 检查是否需要申请存放转码数据的内存
    if (interfaceList.pInterfaceInfoList[int(boardIndex)].nInterfaceType == IMV_FG_EInterfaceType.typeCLInterface):     
        nRet = mallocConvertBuffer(card)
        if (IMV_FG_OK != nRet):
            print("CL mallocConvertBuffer falied.")
    else:
        nRet = mallocConvertBuffer(cam)
        if (IMV_FG_OK != nRet):
